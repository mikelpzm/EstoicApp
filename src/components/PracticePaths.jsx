...
export default function PracticePaths({
  ...
  const [journal, setJournal] = useState(() => localStorage.getItem('stoic:practice-journal') || '');
  ...
  const saveJournal = (value) => {
    setJournal(value);
    localStorage.setItem('stoic:practice-journal', value);
  };
    ...
    <section className="practice-section fade-in">
      ...
      <div className="practice-plan">
        <div>
          ...
          <h3>{activePath.icon} {activePath.title}</h3>
      ...
      <div className="list-header enhanced">
        ...
        <span className="count">{pathMeditations.length} pasajes</span>
      </div>

      <div className="meditations-grid practice-results">
        {pathMeditations.map(meditation => (
          <MeditationCard
            key={meditation.id}
